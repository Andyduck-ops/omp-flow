...
Every native task assignment must name the parent Task ID and the exact expected artifact. Executor/Reviewer assignments must also name the full Row ID; Reviewer assignments must provide a stable native Agent ID for evidence. Batch only work of the same role, use unique deterministic IDs, and treat empty output or a missing promised artifact as failure.

Do not tell a child to redispatch its own role. Workflow-state dispatch instructions are Main-only; child role prompts explicitly treat them as already satisfied.
