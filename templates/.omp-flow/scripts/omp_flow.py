...
def _task_command(args: argparse.Namespace) -> Any:
    ...
    if args.task_action == "rework":
        ...
        if task.get("phase") not in {"ready", "execute", "finish"} or task.get("status") in {"completed", "archived"}:
            raise WorkflowError("Task rework requires a post-freeze task (phase in ready/execute/finish, status not completed/archived)")
...
def _workflow_command(args: argparse.Namespace) -> Any:
    ...
    if args.workflow_action == "select-synthesis":
        ...
        task = read_json(root / "task.json")
        qbd1_status = task.get("gates", {}).get("qbd1", {}).get("status")
        if qbd1_status in {"prepared", "approved"}:
            raise WorkflowError(
                f"Synthesis re-selection is not allowed once qbd1 is {qbd1_status}; "
                "reset qbd1 first if the design intent has changed"
            )
        if task.get("phase") not in {"explore", "design"}:
            raise WorkflowError("Synthesis selection requires phase=explore or phase=design")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    submit.add_argument("--report", help="Path to the review report artifact (defaults to .task/{rowId}.review.md)")
