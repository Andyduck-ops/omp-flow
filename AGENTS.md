...
| 层 | 机制 | 保护对象 |
 ...
 | 2. 激活控制 | `setActiveTools()` 只在 Main session 激活 dispatch；recursion guard 只在拿到明确 `taskDepth > 0` 时拒绝，`mainSessionId` 只做辅助缓存，不作为 hard deny 条件 | 子 agent 无法派发子 agent；新顶层会话不会被旧 session id 误杀 |
