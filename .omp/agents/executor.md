...
If the assembled prompt lacks row ID, boundary contract, task brief, or doneWhen criteria, **do not infer from repository state**. Fail closed and report the missing context to the orchestrator. Guessing the active task or scope from filesystem is forbidden.
