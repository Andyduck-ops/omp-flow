...
- Native OMP model slots: `plan`, with `slow` as fallback for unusually deep architecture decisions.
...
3. **Run QbD 1 audit**: Dispatch an advisor/slow-slot LLM Auditor against `prd.md`, `design.md`, active specs, and context contracts. The audit MUST review boundary rationality, tech selection risk, and spec compliance. On `FAIL`, Architect reads the findings, revises PRD / Design / context contracts, and re-runs QbD 1; loop up to `maxRetries=3`, then escalate to human.
...
10. **Transition FSM**: After human approval gate 2 resolves approved, mark planning complete and transition to dispatch; native `task` handoff will inject Role Definition, Global Context, Curated Context, Task Brief, and Local Guidance through the `tool_call` hook and `.omp-flow/scripts/get_context.py`.
