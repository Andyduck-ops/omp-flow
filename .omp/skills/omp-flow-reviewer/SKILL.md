...
- **Five-layer assembly**: Reviewer receives the same Hook-built prompt stack as executor, with divider lines: `─── omp-flow: Role Definition (from agents/reviewer.md) ───`, `─── omp-flow: Global Context (prd.md + design.md) ───`, `─── omp-flow: Curated Context (ADR / Interface refs from CSV context column) ───`, `─── omp-flow: Task Brief ({rowId}.implement.md) ───`, `─── omp-flow: Local Guidance (Orchestrator) ───`.
...
10. **Complete step**:
   - **Write Markdown report first**: reviewer MUST write `.task/{rowId}.review.md` with sections `Summary`, `Findings (by dimension)`, `Verdict`, and `Evidence`. This is the human-readable semantic body; the host-generated JSON verdict is only the machine-readable header and must match the report.
...
## Outputs
- **Markdown audit report (primary)**: `.task/{rowId}.review.md` — human-readable semantic body with sections `Summary`, `Findings (by dimension)`, `Verdict`, and `Evidence`. The JSON verdict is machine-readable header data generated by the host tool, not a hand-written report replacement.
