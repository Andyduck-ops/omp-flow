/**
 * Canonical task.json shape — single source of truth for OmpFlow tasks.
 *
 * The runtime Python writer is `.omp-flow/scripts/common/task_store.py`
 ...
 * task.json shape.
 */
export interface OmpFlowTaskRecord {
  id: string;
  name: string;
  title: string;
  description: string;
...
export const TASK_RECORD_FIELD_ORDER = [
  ...
  "notes",
  "meta",
] as const satisfies readonly (keyof OmpFlowTaskRecord)[];
...
/**
 * Lightweight runtime schema for {@link OmpFlowTaskRecord}. Zero-dep on
...
export const taskRecordSchema = {
  parse(input: unknown): OmpFlowTaskRecord {
    return parseTaskRecord(input);
  },
  safeParse(
    input: unknown,
  ):
    | { success: true; data: OmpFlowTaskRecord }
...
function parseTaskRecord(input: unknown): OmpFlowTaskRecord {
...
/**
 * Produce a fully-populated canonical-shape {@link OmpFlowTaskRecord}.
 *
...
export function emptyTaskRecord(
  overrides: Partial<OmpFlowTaskRecord> = {},
): OmpFlowTaskRecord {
  const today = new Date().toISOString().split("T")[0] ?? "";
  const base: OmpFlowTaskRecord = {
    id: "",
    name: "",
    title: "",
    description: "",
