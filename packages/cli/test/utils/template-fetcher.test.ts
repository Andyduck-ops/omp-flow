...
describe("getInstallPath", () => {
  it("returns spec path for 'spec' type", () => {
    ...
    expect(result).toBe(path.join("/project", ".omp-flow/spec"));
  });
  ...
  it("works with different cwd values", () => {
    ...
    expect(result).toBe(path.join("/home/user/my-project", ".omp-flow/spec"));
  });
});
...
async function withGitRegistry<T>(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "omp-flow-git-reg-"));
  ...
  try {
    ...
    execFileSync(
      ...
      [
        ...
        "-c",
        "user.email=omp-flow@example.com",
        "commit",
        "-m",
        "fixture",
      ],
...
async function withFakeGit<T>(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "omp-flow-fake-git-"));
...
// NOTE: environment gating via `.runIf` (not `.skip`) so these conditional,
// behavior-preserving guards do NOT collide with the parked-file grep (AC11).
const gitDescribe = describe.runIf(hasGit());
  ...
  it("downloads marketplace templates through the same Git backend", async () => {
    await withGitRegistry(
      ...
      async ({ registry, tmpDir }) => {
        ...
        expect(
          fs.readFileSync(
            path.join(cwd, ".omp-flow", "spec", "rules.md"),
            "utf-8",
          ),
        ).toBe("remote rules\n");
      },
  ...
  it("uses direct download mode when index.json is absent", async () => {
    await withGitRegistry(
      ...
      async ({ registry, tmpDir }) => {
        ...
        const cwd = path.join(tmpDir, "project");
        const specDir = path.join(cwd, ".omp-flow", "spec");
