...
describe("pruneOrphanManifestKeys", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "omp-flow-prune-"));
  ...
  it("preserves every .omp-flow/* entry regardless of platform-collect output", () => {
    const hashes = {
      ".omp-flow/workflow.md": "h1",
      ".omp-flow/scripts/task.py": "h2",
      ".omp-flow/config.yaml": "h3",
    };
  ...
  it("keeps root-level AGENTS.md when it has OmpFlow managed-block markers", () => {
  ...
  it("prunes poisoned root-level AGENTS.md when the file lacks OmpFlow markers", () => {
  ...
  it("does NOT rewrite disk when nothing was pruned", () => {
    const hashes = { ".omp-flow/workflow.md": "h1" };
    saveHashes(tmpDir, hashes);

    const hashFile = path.join(tmpDir, ".omp-flow", ".template-hashes.json");
...
describe("isCwdHomedir / homedir guard helpers", () => {
  ...
  it("homedirBypassEnabled reflects OMP_FLOW_ALLOW_HOMEDIR env var", () => {
    const orig = process.env.OMP_FLOW_ALLOW_HOMEDIR;
    try {
      ...
      for (const value of ["0", "false", "true", ""]) {
        process.env.OMP_FLOW_ALLOW_HOMEDIR = value;
        expect(homedirBypassEnabled()).toBe(false);
      }
    } finally {
      if (orig === undefined) delete process.env.OMP_FLOW_ALLOW_HOMEDIR;
      else process.env.OMP_FLOW_ALLOW_HOMEDIR = orig;
    }
  });
  ...
  it("homedirGuardMessage mentions the command and the bypass env var", () => {
    ...
    expect(msgInit).toContain("OMP_FLOW_ALLOW_HOMEDIR=1");

    const msgUninstall = homedirGuardMessage("uninstall");
    expect(msgUninstall).toContain("uninstall");
    expect(msgUninstall).toContain("OMP_FLOW_ALLOW_HOMEDIR=1");
  });
});
