...
describe("cleanupEmptyDirs", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "omp-flow-cleanup-"));
  });
  ...
  it("[CR#1] does not delete .omp-flow root even if empty", () => {
    fs.mkdirSync(path.join(tmpDir, ".omp-flow"), { recursive: true });
    cleanupEmptyDirs(tmpDir, ".omp-flow");
    expect(fs.existsSync(path.join(tmpDir, ".omp-flow"))).toBe(true);
  });

  it("recursively cleans parent directories but stops at root", () => {
    // Create .omp-flow/scripts/multi_agent/ (all empty)
    fs.mkdirSync(path.join(tmpDir, ".omp-flow", "scripts", "multi_agent"), {
      recursive: true,
    });
    cleanupEmptyDirs(tmpDir, ".omp-flow/scripts/multi_agent");

    // multi_agent and scripts should be removed (both empty)
    expect(
      fs.existsSync(
        path.join(tmpDir, ".omp-flow", "scripts", "multi_agent"),
      ),
    ).toBe(false);
    expect(
      fs.existsSync(path.join(tmpDir, ".omp-flow", "scripts")),
    ).toBe(false);
    // .omp-flow root must survive
    expect(fs.existsSync(path.join(tmpDir, ".omp-flow"))).toBe(true);
  });
...
describe("loadUpdateSkipPaths", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "omp-flow-skip-"));
    fs.mkdirSync(path.join(tmpDir, ".omp-flow"), { recursive: true });
  });
  ...
  it("strips double quotes from skip paths", () => {
    fs.writeFileSync(
      path.join(tmpDir, ".omp-flow", "config.yaml"),
      'update:\n  skip:\n    - ".claude/commands/"\n',
    );
...
describe("sortMigrationsForExecution", () => {
  ...
  it("puts rename-dir before rename and delete", () => {
    const items = [
      ...
      { type: "rename-dir" as const, from: ".omp-flow/old", to: ".omp-flow/new" },
  ...
  it("sorts rename-dir by path depth (deeper first)", () => {
    const items = [
      { type: "rename-dir" as const, from: ".omp-flow/a", to: ".omp-flow/x" },
      {
        type: "rename-dir" as const,
        from: ".omp-flow/a/b/c",
        to: ".omp-flow/x/y/z",
      },
      { type: "rename-dir" as const, from: ".omp-flow/a/b", to: ".omp-flow/x/y" },
    ];
    const sorted = sortMigrationsForExecution(items);
    expect(sorted[0].from).toBe(".omp-flow/a/b/c"); // depth 4
    expect(sorted[1].from).toBe(".omp-flow/a/b"); // depth 3
    expect(sorted[2].from).toBe(".omp-flow/a"); // depth 2
  });
...
describe("shouldExcludeFromBackup", () => {
  ...
  it.each([
    ...
    ".omp-flow/.backup-2026-04-22T10-24-27/.opencode/node_modules/zod/index.js",
  ])("excludes dependency tree %s", (p) => {
    expect(shouldExcludeFromBackup(p)).toBe(true);
  });

  it.each([
    ".omp-flow/workspace/developer/journal-1.md",
    ".omp-flow/tasks/04-17-foo/prd.md",
    ".omp-flow/spec/cli/backend/index.md",
    ".omp-flow/backlog/idea.md",
    ".omp-flow/agent-traces/trace.jsonl",
  ...
  it("excludes previous backups", () => {
    expect(
      shouldExcludeFromBackup(".omp-flow/.backup-2026-04-20T01-00-00/x"),
    ).toBe(true);
  });

  it.each([
    ".claude/commands/omp-flow/continue.md",
    ".claude/skills/omp-flow-check/SKILL.md",
    ".omp-flow/workflow.md",
    ".omp-flow/scripts/get_context.py",
    ".agents/skills/omp-flow-check/SKILL.md",
  ...
  // after normalization, otherwise OmpFlow's native worktree protection
  ...
  // `omp-flow update --migrate`, late April 2026).
  it.each([
    ".claude\\worktrees\\feat-x\\src\\main.ts",
    ".omp-flow\\tasks\\04-17-foo\\prd.md",
    ".omp-flow\\workspace\\dev\\journal-1.md",
