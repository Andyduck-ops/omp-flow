...
function setup(): TmpEnv {
  ...
  const oldRoot = process.env.OMP_FLOW_CHANNEL_ROOT;
  const oldProject = process.env.OMP_FLOW_CHANNEL_PROJECT;
  process.env.OMP_FLOW_CHANNEL_ROOT = channelsRoot;
  delete process.env.OMP_FLOW_CHANNEL_PROJECT;
  return {
    tmpDir,
    projectDir,
    channelsRoot,
...
function teardown(env: TmpEnv): void {
  if (env.oldRoot === undefined) delete process.env.OMP_FLOW_CHANNEL_ROOT;
  else process.env.OMP_FLOW_CHANNEL_ROOT = env.oldRoot;
  if (env.oldProject === undefined) delete process.env.OMP_FLOW_CHANNEL_PROJECT;
  else process.env.OMP_FLOW_CHANNEL_PROJECT = env.oldProject;
...
describe("scanLiveWorkers + enforceSpawnBudget (integration)", () => {
  ...
  it("enforceSpawnBudget rejects when budget is still exhausted post-cleanup", async () => {
    ...
    expect(msg).toContain("omp-flow channel kill");
