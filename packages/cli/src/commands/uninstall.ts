/**
 * `omp-flow uninstall` — remove every file written by `omp-flow init` / `update`
 * from the current project, plus the `.omp-flow/` directory itself.
 *
 * The single source of truth for "what omp-flow wrote" is
 * `.omp-flow/.template-hashes.json`. Files outside that manifest are never
 ...
 *      package.json) — passed through a scrubber that strips just the omp-flow
 ...
 * (per the PRD: "全删"). The `.omp-flow/` tree is removed unconditionally.
 */
...
interface StructuredFileSpec {
   ...
   * omp-flow-managed `command` strings.
   */
  ...
  const specs: StructuredFileSpec[] = [
    ...
    ...(
      ...
      (p): StructuredFileSpec => ({
        posixPath: p,
        reason: "Strip omp-flow hooks; preserve user fields",
    ...
    },
    {
      ...
      reason:
        "Strip omp-flow extension/skills/prompts entries; preserve user fields",
      scrub: (content) => scrubPiSettings(content),
    },
    {
      posixPath: ".codex/config.toml",
      reason: "Remove omp-flow project_doc_fallback_filenames and notes",
      scrub: (content) => scrubCodexConfigToml(content),
    },
    {
      posixPath: COPILOT_INSTRUCTIONS_PATH,
      reason: "Remove OmpFlow Copilot guidance; preserve repo instructions",
      scrub: (content) =>
        scrubManagedMarkdownBlock(
          content,
          COPILOT_INSTRUCTIONS_BLOCK_START,
...
interface UninstallPlan {
  ...
  /** Whether `.omp-flow/` directory itself will be removed. */
  removeOmpFlowDir: boolean;
}
...
function renderPlan(cwd: string, plan: UninstallPlan): void {
  const ompFlowDir = path.join(cwd, DIR_NAMES.WORKFLOW);

  console.log(chalk.bold("\nOmpFlow uninstall plan\n"));
  ...
  if (plan.removeOmpFlowDir && fs.existsSync(ompFlowDir)) {
    console.log(
...
/**
 * Execute the plan: write modifications, unlink deletions, remove `.omp-flow/`,
...
function executePlan(
  ...
  // 3. Drop `.omp-flow/` entirely.
  let deletedDirs = 0;
  if (plan.removeOmpFlowDir) {
    const ompFlowDir = path.join(cwd, DIR_NAMES.WORKFLOW);
    if (fs.existsSync(ompFlowDir)) {
      fs.rmSync(ompFlowDir, { recursive: true, force: true });
      deletedDirs += 1;
    }
  }
  ...
  // has no purpose. `.omp-flow` is already gone (step 3), so we skip it.
...
export async function uninstall(options: UninstallOptions = {}): Promise<void> {
  ...
  // Pre-check 1: must have a `.omp-flow/` directory.
  if (!fs.existsSync(ompFlowDir)) {
    console.log(
      chalk.gray(
        "OmpFlow is not installed in this project (no .omp-flow/ directory found).",
      ),
    );
    return;
  }
  ...
  // platform files are omp-flow-owned vs user-owned.
  const hashes = loadHashes(cwd);
  if (Object.keys(hashes).length === 0) {
    console.error(
      chalk.red(
        "OmpFlow directory found but manifest is missing — cannot determine which platform files to remove. " +
          "You can manually delete .omp-flow/ if needed.",
      ),
    );
    process.exit(1);
  }
  ...
  // manifest only to delete the whole .omp-flow/ dir anyway — but the
  ...
  if (pruned.length > 0) {
    ...
    console.log(
      chalk.gray(
        `   Pruned ${pruned.length} orphan manifest entries (user-owned files omp-flow did not write).`,
      ),
    );
  }
  ...
  if (!options.yes) {
    ...
    // honor the same UX as `omp-flow update` (which also fails closed in
    // that case).
  ...
  console.log(
    chalk.green(
      `Uninstalled omp-flow: ${summary.deletedFiles} files deleted, ` +
