...
function binaryCheckCommand(
  ...
  return platform === "win32" ? "where omp-flow" : "which omp-flow";
}
...
function troubleshooting(plan: UpgradeCommandPlan): string {
  return [
    ...
    `- Check which OmpFlow binary your shell resolves: ${plan.binaryCheckCommand}`,
    "- If this is a permissions error, fix your Node/npm install or npm prefix; OmpFlow does not run sudo.",
    "- If npm reports an existing binary or locked file, resolve that npm error manually; OmpFlow does not run --force.",
  ].join("\n");
}
...
export async function upgrade(
  ...
  console.log(chalk.cyan(`Upgrading OmpFlow CLI to ${plan.target}`));
  ...
  console.log(chalk.green("\n✓ OmpFlow CLI upgrade completed"));
  console.log(chalk.gray("Run: omp-flow --version"));
