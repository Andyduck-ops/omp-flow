...
description: "Adapts an external skill into project-specific development guidelines in .omp-flow/spec/. Creates guideline sections, code example templates with .template suffix, and updates spec indexes. Use when integrating an external skill, adding a new skill's patterns to project conventions, or incorporating third-party skill best practices into .omp-flow/spec/ documentation."
---
...
>
> - Guidelines content -> Write to `.omp-flow/spec/{target}/doc.md`
> - Code examples -> Place in `.omp-flow/spec/{target}/examples/skills/<skill-name>/`
...
|----------------|-------------------|
| UI/Frontend (`frontend-design`, `web-artifacts-builder`) | `.omp-flow/spec/frontend/` |
| Backend/API (`mcp-builder`) | `.omp-flow/spec/backend/` |
| Documentation (`doc-coauthoring`, `docx`, `pdf`) | `.omp-flow/` or create dedicated guidelines |
| Testing (`webapp-testing`) | `.omp-flow/spec/frontend/` (E2E) |
...
- **Integration target**: `.omp-flow/spec/{target}/`
...
| Type | Path |
|------|------|
| Guidelines doc | `.omp-flow/spec/{target}/doc.md` (section: `skill-<name>`) |
| Code examples | `.omp-flow/spec/{target}/examples/skills/<name>/` |
| Index update | `.omp-flow/spec/{target}/index.md` |
...
| `doc-coauthoring` | `.omp-flow/` | N/A (documentation workflow only) |
