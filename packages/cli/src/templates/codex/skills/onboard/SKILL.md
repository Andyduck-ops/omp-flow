---
name: onboard
description: "Interactive three-part onboarding for new team members to the OmpFlow AI-assisted workflow system. Covers core philosophy (AI memory, project-specific knowledge, context drift), system structure and command deep-dives, real-world workflow examples, and guideline customization. Use when a new developer joins the project, someone needs to understand the OmpFlow workflow, or project guidelines need initial setup."
---
...
**The Solution**: The `.omp-flow/workspace/` system captures what happened in each session - what was done, what was learned, what problems were solved. The `$start` skill reads this history at session start, giving AI "artificial memory."
...
**The Solution**: The `.omp-flow/spec/` directory contains project-specific guidelines. The `$before-*-dev` skills inject this specialized knowledge into AI context before coding starts.
...
**[2/8] python3 ./.omp-flow/scripts/omp_flow.py task create "Fix bug" --slug fix-bug** - Track work for future reference
...
**[2/4] python3 ./.omp-flow/scripts/omp_flow.py task create "Planning task" --slug planning-task** - Planning is valuable work
...
Check if `.omp-flow/spec/` contains empty templates or customized guidelines:

```bash
# Check if files are still empty templates (look for placeholder text)
grep -l "To be filled by the team" .omp-flow/spec/backend/*.md 2>/dev/null | wc -l
grep -l "To be filled by the team" .omp-flow/spec/frontend/*.md 2>/dev/null | wc -l
```
...
If guidelines are empty templates (contain "To be filled by the team"), this is the first time using OmpFlow in this project.

Explain to the developer:

"I see that the development guidelines in `.omp-flow/spec/` are still empty templates. This is normal for a new OmpFlow setup!
...
For example, for `.omp-flow/spec/backend/database-guidelines.md`:
...
I recommend reading through `.omp-flow/spec/` to familiarize yourself with the team's coding standards."
...
```bash
python3 ./.omp-flow/scripts/omp_flow.py task create "Fill spec guidelines" --slug fill-spec-guidelines
```
...
"You're now onboarded to the OmpFlow workflow system! Here's what we covered:
...
2. [If guidelines empty] Start filling in `.omp-flow/spec/` guidelines
