 ...
 *
 * Running `omp-flow init` / `omp-flow uninstall` in `$HOME` is catastrophic:
...
/**
 * Error message printed by both `omp-flow init` and `omp-flow uninstall` when
 * the homedir guard trips.
 */
export function homedirGuardMessage(commandName: "init" | "uninstall"): string {
  return (
    `✗ Refusing to run \`omp-flow ${commandName}\` in your home directory.\n\n` +
    `OmpFlow manages platform config dirs like .claude/, .codex/, .opencode/, which\n` +
    ...
    `Run omp-flow from your project directory instead. If you really want to run in\n` +
    `$HOME, set OMP_FLOW_ALLOW_HOMEDIR=1.`
  );
}
...
export function homedirBypassEnabled(): boolean {
  return process.env.OMP_FLOW_ALLOW_HOMEDIR === "1";
}
