...
async function main() {
  ...
  try {
    ...
    if (versionOnNpm(version)) {
      console.error(
        `\n✗ Version ${version} is already published on npm.\n` +
        `  Rewriting its manifest would break \`omp-flow update\` for users\n` +
    ...
    if (breaking && recommendMigrate) {
      ...
      migrationGuide = "TODO: narrative migration guide (gets templated into the user's migration task PRD on omp-flow update --migrate).";
