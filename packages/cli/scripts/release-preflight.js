 ...
 *   1. Version match between `omp-flow` and
 *      `omp-flow-core` (and the current git tag when checked from
 *      a tag context).
...
function verifyPackedCli() {
  ...
  try {
    ...
    const dep = packedPkg.dependencies?.["omp-flow-core"];
    if (!dep) {
      fail(`packed CLI is missing dependency on omp-flow-core.`);
    }
    if (dep !== v.cliVersion) {
      fail(
        `packed CLI depends on omp-flow-core@"${dep}" but expected exact "${v.cliVersion}".\n` +
          `pnpm should rewrite workspace:* to the exact published version; got "${dep}" instead.`,
      );
    }
    console.log(
      `${GREEN}ok${RESET} packed CLI pins omp-flow-core to exact ${v.cliVersion}.`,
    );
  } finally {
