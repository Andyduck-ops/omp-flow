...
2. **二级库 (Tier 2 Digested References)**：`omp-flow-researcher` 识别值得复用的一级库源码锚点，`omp_flow_reference` / `ReferenceDigester` 执行“消化（Digestion）”，把最关键的代码切片与配置保存至 Task 专属 `reference/` 目录，每条结论附带一级库的 `file:line` 物理锚点。
